...
func (c *CPUSubsystem) ApplySettings(cgroupPath string, resources *Resources) error {
	if err := setSubsystemValue(c.fileHandler, cgroupPath, "cpu.shares", resources.CPU.Shares); err != nil {
		zap.L().Error("Failed to apply CPU settings", zap.String("subsystem", "cpu"), zap.String("cgroupPath", cgroupPath), zap.Error(err))
		return fmt.Errorf("failed to apply CPU settings: %w", err)
	}
	return nil
}
...
func (m *MemorySubsystem) ApplySettings(cgroupPath string, resources *Resources) error {
	if err := setSubsystemValue(m.fileHandler, cgroupPath, "memory.limit_in_bytes", resources.Memory.Limit); err != nil {
		zap.L().Error("Failed to apply memory settings", zap.String("subsystem", "memory"), zap.String("cgroupPath", cgroupPath), zap.Error(err))
		return fmt.Errorf("failed to apply memory settings: %w", err)
	}
	return nil
}
...
func (b *BlkIOSubsystem) ApplySettings(cgroupPath string, resources *Resources) error {
	if err := setSubsystemValue(b.fileHandler, cgroupPath, "blkio.weight", resources.BlkIO.Weight); err != nil {
		zap.L().Error("Failed to apply block I/O settings", zap.String("subsystem", "blkio"), zap.String("cgroupPath", cgroupPath), zap.Error(err))
		return fmt.Errorf("failed to apply block I/O settings: %w", err)
	}
	return nil
}
